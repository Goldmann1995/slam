...
#include "lio_sam/SetGlobalMap.h"
#include "lio_sam/SetGlobalLocalizationEngine.h"
#include "lio_sam/QueryGlobalLocalization.h"
...
public:
    ...
    std::unique_ptr<hdl_global_localization::GlobalLocalizationEngine> engine;
    ...
    pcl::Filter<PointType>::Ptr downsample_filter;
    // pose estimator
    ...
    pcl::PointCloud<PointType>::Ptr points_raw;
    ...
    pcl::VoxelGrid<PointType> downSizeFilter_Points_Raw;
	...
	//std::cout << "come in2" << std::endl;
        ...
        downSizeFilter_Points_Raw.setLeafSize(mappingSurfLeafSize, mappingSurfLeafSize, mappingSurfLeafSize);

        // publish globalmap with "latched" publisher
        initialize_params_globalmap();
        globalmap_pub = nh.advertise<sensor_msgs::PointCloud2>("/globalmap", 5, true);
        map_update_sub = nh.subscribe("/map_request/pcd", 10,              &mapOptimization::map_update_callback, this);

        globalmap_pub_timer = nh.createWallTimer(ros::WallDuration(1.0), &mapOptimization::pub_once_cb, this, true, true);

        globalmap_sub = nh.subscribe("/globalmap", 1, &mapOptimization::globalmap_callback, this);
        initialpose_pub = nh.advertise< geometry_msgs::PoseWithCovarianceStamped>("/initialpose", 5, true);
        // global localization
        initialize_params_localization(); 
        
        // global localization
        use_global_localization = nh.param<bool>("use_global_localization", true);
        if(use_global_localization) {
        ROS_INFO_STREAM("wait for global localization services");
        ros::service::waitForService("/lio_sam/set_global_map");
        ros::service::waitForService("/lio_sam/query");

        set_global_map_service = nh.serviceClient< lio_sam::SetGlobalMap>("/lio_sam/set_global_map");
        query_global_localization_service = nh.serviceClient< lio_sam::QueryGlobalLocalization>("/lio_sam/query");

        relocalize_server = nh.advertiseService("/relocalize", &mapOptimization::relocalize, this);
        }
        allocateMemory();
    }
...
//   }
    ...
    //     if (engine_name == "BBS") {
    //     engine.reset(new hdl_global_localization::GlobalLocalizationBBS(nh));
    //     } else if (engine_name == "FPFH_RANSAC") {
    //     engine.reset(new hdl_global_localization::GlobalLocalizationEngineFPFH_RANSAC(nh));
    //     }
    // #ifdef TEASER_ENABLED
    //     else if (engine_name == "FPFH_TEASER") {
    //     engine.reset(new hdl_global_localization::GlobalLocalizationEngineFPFH_Teaser(nh));
    //     }
    // #endif
    //     else {
...
//     bool set_engine( lio_sam::SetGlobalLocalizationEngine::Request& req,  lio_sam::SetGlobalLocalizationEngine::Response& res) {
//         ROS_INFO_STREAM("Set Global Localization Engine");
//         return set_engine(req.engine_name.data);
//     }

//   bool set_global_map( lio_sam::SetGlobalMapRequest& req, lio_sam::SetGlobalMapResponse& res) {
//     ROS_INFO_STREAM("Global Map Received");

//     pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
//     pcl::fromROSMsg(req.global_map, *cloud);
//     cloud = downsample(cloud, nh.param<double>("globalmap_downsample_resolution", 0.5));
...
//  bool query( lio_sam::QueryGlobalLocalization::Request& req,  lio_sam::QueryGlobalLocalization::Response& res) {
...
//     cloud = downsample(cloud, nh.param<double>("query_downsample_resolution", 0.5));
...
//   }
    ...
    pcl::Registration<PointType, PointType>::Ptr create_registration() const {
    std::string reg_method = nh.param<std::string>("reg_method", "NDT_OMP");
    std::string ndt_neighbor_search_method = nh.param<std::string>("ndt_neighbor_search_method", "DIRECT7");
    double ndt_neighbor_search_radius = nh.param<double>("ndt_neighbor_search_radius", 2.0);
    double ndt_resolution = nh.param<double>("ndt_resolution", 1.0);

    if(reg_method == "NDT_OMP") {
      ROS_INFO_STREAM("NDT_OMP is selected");
      ...
      if (ndt_neighbor_search_method == "DIRECT1") {
        ROS_INFO_STREAM("search_method DIRECT1 is selected");
        ndt->setNeighborhoodSearchMethod(pclomp::DIRECT1);
      } else if (ndt_neighbor_search_method == "DIRECT7") {
        ROS_INFO_STREAM("search_method DIRECT7 is selected");
        ndt->setNeighborhoodSearchMethod(pclomp::DIRECT7);
      } else {
        if (ndt_neighbor_search_method == "KDTREE") {
          ROS_INFO_STREAM("search_method KDTREE is selected");
        } else {
    ...
    //   ROS_INFO_STREAM("NDT_CUDA is selected");
    //   boost::shared_ptr<fast_gicp::NDTCuda<PointType, PointType>> ndt(new fast_gicp::NDTCuda<PointType, PointType>);
    //   ndt->setResolution(ndt_resolution);
    ...
    //     ROS_INFO_STREAM("search_method DIRECT_RADIUS is selected : " << ndt_neighbor_search_radius);
    ...
    ROS_ERROR_STREAM("unknown registration method:" << reg_method);
    return nullptr;
  }
  ...
  bool relocalize(std_srvs::EmptyRequest& req, std_srvs::EmptyResponse& res) {
    ...
    downSizeFilter_Points_Raw.setInputCloud(points_raw);
    downSizeFilter_Points_Raw.filter(*laserCloudIn);
    
    if(laserCloudIn == nullptr) {
      ROS_INFO_STREAM("no scan has been received");
      return false;
    }
    relocalizing = true;
    delta_estimater->reset();
    pcl::PointCloud<PointType>::Ptr scan = laserCloudIn;

    lio_sam::QueryGlobalLocalization srv;
    pcl::toROSMsg(*scan, srv.request.cloud);
    srv.request.max_num_candidates = 1;
    std::cout << "!query_global_localization_service.call(srv)"<< !query_global_localization_service.call(srv)<<std::endl;
    std::cout <<" srv.response.poses.empty()"<< srv.response.poses.empty()<<std::endl;

    if(!query_global_localization_service.call(srv) || srv.response.poses.empty()) {
      relocalizing = false;
      ROS_INFO_STREAM("global localization failed");
      return false;
    }

    const auto& result = srv.response.poses[0];

    ROS_INFO_STREAM("--- Global localization result ---");
    ROS_INFO_STREAM("Trans :" << result.position.x << " " << result.position.y << " " << result.position.z);
    ROS_INFO_STREAM("Quat  :" << result.orientation.x << " " << result.orientation.y << " " << result.orientation.z << " " << result.orientation.w);
    ROS_INFO_STREAM("Error :" << srv.response.errors[0]);
    ROS_INFO_STREAM("Inlier:" << srv.response.inlier_fractions[0]);
    ...
    std::cout << "pose_estimator->matrix().at(0,3)" << pose_estimator->matrix()(0,3) <<std::endl; 
    std::cout << "pose_estimator->matrix().at(1,3)" << pose_estimator->matrix()(1,3) <<std::endl; 
    std::cout << "pose_estimator->matrix().at(2,3)" << pose_estimator->matrix()(2,3) <<std::endl; 


    //  Eigen::Matrix3f  RotationMatrix =   pose_estimator->matrix().block(0, 0, 3, 3) ;
    //  geometry_msgs::PoseWithCovarianceStamped pose_odomTo_map;

    // Eigen::Quaternionf q_odomTo_map =     Eigen::Quaternionf( RotationMatrix);
        ...
        icp.align(*unused_result,pose_estimator->matrix());
    ...
    Eigen::Matrix4f  T = transOdomToMap.matrix();

    Eigen::Matrix3f  RotationMatrix =   T.block(0, 0, 3, 3) ;
    geometry_msgs::PoseWithCovarianceStamped pose_odomTo_map;

    Eigen::Quaternionf q_odomTo_map =     Eigen::Quaternionf( RotationMatrix);
     pose_odomTo_map.header.stamp = timeLaserInfoStamp;
     pose_odomTo_map.header.frame_id = "map";
    std::cout << "T_thisPose6DInMap.translation()(0)" << T_thisPose6DInMap.translation()(0)<<std::endl; 
        std::cout << "T_thisPose6DInMap.translation()(1)" << T_thisPose6DInMap.translation()(1)<<std::endl; 
            std::cout << "T_thisPose6DInMap.translation()(2)" <<T_thisPose6DInMap.translation()(2) <<std::endl; 

    // pose_odomTo_map.pose.pose.position.x = pose_estimator->matrix()(0,3);
    // pose_odomTo_map.pose.pose.position.y = pose_estimator->matrix()(1,3); 
    // pose_odomTo_map.pose.pose.position.z =pose_estimator->matrix()(2,3);
    // pose_odomTo_map.pose.pose.orientation.x = q_odomTo_map.x();
    // pose_odomTo_map.pose.pose.orientation.y = q_odomTo_map.y();
    // pose_odomTo_map.pose.pose.orientation.z = q_odomTo_map.z();
    // pose_odomTo_map.pose.pose.orientation.w = q_odomTo_map.w();

    pose_odomTo_map.pose.pose.position.x = T(0,3);
    pose_odomTo_map.pose.pose.position.y = T(1,3); 
    pose_odomTo_map.pose.pose.position.z = T(2,3);
    pose_odomTo_map.pose.pose.orientation.x = q_odomTo_map.x();
    pose_odomTo_map.pose.pose.orientation.y = q_odomTo_map.y();
    pose_odomTo_map.pose.pose.orientation.z = q_odomTo_map.z();
    pose_odomTo_map.pose.pose.orientation.w = q_odomTo_map.w();
    ...
    // pcl::transformPointCloud(*laserCloudIn, *laserCloudIn, pose_estimator->matrix());
    // publishCloud(&pubLaserCloudInWorld, laserCloudIn, timeLaserInfoStamp, "map");
    initialpose_pub.publish(pose_odomTo_map);
    relocalizing = false;
  ...
  void initialize_params_localization() {
    ...
    ROS_INFO_STREAM("create registration method for localization");
    registration = create_registration();

    // global localization
    ROS_INFO_STREAM("create registration method for fallback during relocalization");
    relocalizing = false;
    delta_estimater.reset(new DeltaEstimater(create_registration()));

    // initialize pose estimator
    if(nh.param<bool>("specify_init_pose", true)) {
      ROS_INFO_STREAM("initialize pose estimator with specified parameters!!");
    geometry_msgs::PoseWithCovarianceStamped pose_odomTo_map;
     pose_odomTo_map.header.stamp = timeLaserInfoStamp;
     pose_odomTo_map.header.frame_id = "map";
    pose_odomTo_map.pose.pose.position.x = nh.param<double>("init_pos_x", 0.0);
    pose_odomTo_map.pose.pose.position.y = nh.param<double>("init_pos_y", 0.0); 
    pose_odomTo_map.pose.pose.position.z = nh.param<double>("init_pos_z", 0.0);
    pose_odomTo_map.pose.pose.orientation.x = nh.param<double>("init_ori_x", 0.0);
    pose_odomTo_map.pose.pose.orientation.y = nh.param<double>("init_ori_y", 0.0);
    pose_odomTo_map.pose.pose.orientation.z = nh.param<double>("init_ori_z", 0.0);
    pose_odomTo_map.pose.pose.orientation.w = nh.param<double>("init_ori_w", 1.0);
    initialpose_pub.publish(pose_odomTo_map);
    ...
    void initialize_params_globalmap() {
    // read globalmap from a pcd file
    std::string globalmap_pcd = nh.param<std::string>("globalmap_pcd", "/home/jzc/Downloads/LOAM/cloudGlobal.pcd");
    globalmap.reset(new pcl::PointCloud<PointType>());
    pcl::io::loadPCDFile(globalmap_pcd, *globalmap);
    globalmap->header.frame_id = "map";

    std::ifstream utm_file(globalmap_pcd + ".utm");
    if (utm_file.is_open() && nh.param<bool>("convert_utm_to_local", true)) {
      double utm_easting;
      double utm_northing;
      double altitude;
   ...
   * @param points_msg
   */
  void globalmap_callback(const sensor_msgs::PointCloud2Ptr& points_msg) {
    ROS_INFO_STREAM("globalmap received!");
    ...
    if(use_global_localization) {
      ROS_INFO_STREAM("set globalmap for global localization!");
      lio_sam::SetGlobalMap srv;
    ...
    void resetLIO()
    {
        ...
        points_raw.reset(new pcl::PointCloud<PointType>());
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        pcl::fromROSMsg(msgIn->cloud_deskewed, *points_raw);
        /************************************added by gc*****************************/
