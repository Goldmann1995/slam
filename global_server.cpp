...
private:
  ...
  bool set_engine(const std::string& engine_name) {
    if (engine_name == "BBS") {
      engine.reset(new hdl_global_localization::GlobalLocalizationBBS(private_nh));
    } else if (engine_name == "FPFH_RANSAC") {
      engine.reset(new hdl_global_localization::GlobalLocalizationEngineFPFH_RANSAC(private_nh));
    }
#ifdef TEASER_ENABLED
    else if (engine_name == "FPFH_TEASER") {
      engine.reset(new hdl_global_localization::GlobalLocalizationEngineFPFH_Teaser(private_nh));
    }
#endif
    else {
...
int main(int argc, char** argv) {
  ...
  lio_sam::GlobalLocalizationNode node;
  ros::spin();
